...
\citeauthor{Chan2003} described the texture using the frequency signature via the \acf{dct} \cite{Ahmed1974} defining a neighbourhood of $7 \times 7$ pixels for each of the modalities that they used.
...
\citeauthor{Viswanath2012} projected \ac{t2w} images into the wavelet space, using Haar wavelet, and used the coefficients obtained from the decomposition as features.
%The wavelet family used for the decomposition was the Haar wavelet.

Litjens \textit{et al.} in \cite{Litjens2011} computed the texture map based on \ac{t2w} images using a Gaussian filer bank.
...
The authors proposed to calculate four asymmetry features by divinding a region into two parts along its axes and compare the areas. 
\begin{equation}
