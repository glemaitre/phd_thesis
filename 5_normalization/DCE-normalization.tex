...
First Sect.~\ref{subsec:chp5:DCE-norm:norm} details our normalization strategy for \ac{dce}-\ac{mri} data.
Quantitative and semi-quantitative methods are summarized in Sect.\,\ref{subsec:chp5:DCE-norm:stateart} with insights about their implementations.
Finally experiments and results to answer the previous stated challenges are reported in Sect.\,\ref{subsec:chp5:DCE-norm:exp} while discussed in Sect.\,\ref{subsec:chp5:DCE-norm:discussions}, followed by a concluding section.
...
In this section, we summarize the different methods which have been used for the quantification of \ac{dce}-\ac{mri} for \ac{cap} detection~\citep{Lemaitre2015} and which will be used for comparison in this work.
...
Hoffman \textit{at al.}~\cite{hoffmann1995pharmacokinetic} proposed a similar model as expressed in Eq.\,\eqref{eq:hoffmann}, which derive from the Brix model:
...
\section{Experiment and results}\label{sec:chp5:DCE-norm:exp-res}
