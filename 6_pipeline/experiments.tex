...
We attend to find the most discriminative ``quantification'' methods in terms of classification performance to extract information of \ac{dce}-\ac{mri} modality.
Therefore, the pharmacokinetic parameters from the Brix, Hoffmann, Tofts, and \ac{pun} models as well as the semi-quantitative parameters and the enhanced signal are separately used for training different \ac{rf} classifiers.

\paragraph{\ac{mrsi} features} Similarly to \ac{dce}-\ac{mri}, four \ac{rf} classifiers are trained on different features:
...
(iv) the ratio of the relative concentration of the citrate over the relative concentration of the choline, using the fix bounds integration.

\paragraph{Results}

\begin{figure}
  \hspace*{\fill}
  \subfigure[Performance of the quantitative methods on \acs*{dce}-\acs*{mri}.]{\label{fig:inddcemodel}\includegraphics[width=.49\textwidth]{5_normalization/figures/DCE-normalization/normalized_methods_0.pdf}}
  \hfill
  \subfigure[Performance of enhanced \acs*{dce}-\acs*{mri} signal.]{\label{fig:inddcesignal}\includegraphics[width=.49\textwidth]{5_normalization/figures/DCE-normalization/full_signal_0.pdf}}
  \hspace*{\fill} \\
  \hspace*{\fill}
  \subfigure[Performance of image-based features for \acs*{t2w}-\acs*{mri} and \acs*{adc} map.]{\label{fig:indadct2w}\includegraphics[width=.49\textwidth]{6_pipeline/figures/exp-1/t2w_adc.pdf}}
  \hfill
  \subfigure[Performance of different approach for the \acs*{mrsi} modality.]{\label{fig:indmrsi}\includegraphics[width=.49\textwidth]{6_pipeline/figures/exp-1/mrsi_all.pdf}}
  \hspace*{\fill}
...
Each trained \ac{rf} is evaluated using a \ac{lopo} cross-validation.
A \ac{roc} analysis with its corresponding \ac{auc} is carried out to report the classification performance of each classifier.
